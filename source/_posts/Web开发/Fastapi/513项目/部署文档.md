...
* 在项目根目录下创建 `docker-compose.yml` 文件，用于定义整个项目的 Docker 服务配置：

/home/ldsz/513-API/docker-compose.yml
```yml
services:
  mysql:
    image: mysql:8.0
  ...
  celery_worker:
    ...
    command: celery -A app.core.tasks worker --loglevel=info --concurrency=100
    env_file:
      - .env
    depends_on:
      - redis
...
docker compose --profile celery_beat --profile celery_worker up -d 
