...
git checkout -t origin/lwj_test
...
##### 1、切换 Git 分支到远程的 test 分支

要将本地分支从 master 切换到远程仓库的 test 分支，您需要执行以下步骤：

方法一：直接创建并切换到远程 test 分支的本地跟踪分支
```bash
# 首先，获取远程仓库的最新信息
git fetch origin

# 创建并切换到本地 test 分支，同时将其设置为跟踪远程的 test 分支
git checkout -t origin/test

# 或者使用更明确的命令
git checkout -b test origin/test
git checkout -b feature/buju origin/feature/buju
...
# 3. 创建本地 test 分支并切换到该分支
git checkout -b test

# 4. 将本地 test 分支与远程 test 分支关联
git branch -u origin/test test

# 或者在第3步直接创建跟踪分支
git checkout -b test origin/test
```
方法三：如果远程 test 分支已存在本地副本
```bash
# 直接切换到本地已存在的 test 分支
git checkout test

# 如果本地分支已存在但不是跟踪远程分支，可以设置上游
