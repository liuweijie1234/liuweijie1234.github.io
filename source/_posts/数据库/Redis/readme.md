...
深挖ZSet底层跳表、持久化RDB/AOF混合策略、大Key热Key治理、分布式锁的Redlock算法缺陷。


缓存一致性：先更新DB还是先删缓存？为什么是“删缓存”而不是“更新缓存”？旁路缓存模式（Cache-Aside）的缺陷是什么？

5 大数据结构底层、持久化 RDB/AOF、缓存穿透 / 击穿 / 雪崩解决方案、分布式锁、过期策略、集群主从、Redis 事务
